...
Creare il “Database” (struttura dati) e la visualizzazione di queste domande e risposte con PHP. -->
<a href='#'></a>
<!-- DATABASE -->
<?php 
    $texts_google_faq = [
...
<body>
    ...
    <header>
        <!-- first raw header -->
        <div id="first-row-header" class="d-flex">
            <div>
    ...
    <main>
        <div class="container">
            <?php foreach($texts_google_faq as $number_of_text => $text) { ?>
                <h3><?php echo $text["title"];  ?></h3>
                <?php foreach($text["paragraph"] as $single_paragraph) { ?>
                    <p><?php echo $single_paragraph; ?></p> 
                <?php } ?>
            <?php } ?>
        </div>
